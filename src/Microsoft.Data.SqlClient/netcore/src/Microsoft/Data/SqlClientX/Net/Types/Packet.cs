...
namespace Microsoft.Data.SqlClientX.Net.Types
{
    internal class Packet
    {
        ...
        internal readonly Messenger _messenger; // used in debug builds to check that packets are being returned to the correct pool
        ...
        public Packet(Messenger messenger, int id)
            : this()
        {
#if TRACE_HISTORY
            ...
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} instantiated,", args0: _messenger?.ConnectionId, args1: _id);
        }
        ...
        ~Packet()
        {
            if (_data != null)
            {
                SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.ERR, "Finalizer called for unreleased Packet, Connection Id {0}, Packet Id {1}, _refCount {2}, DataLeft {3}, tag {4}", args0: _messenger?.ConnectionId, args1: _id, args2: _refCount, args3: DataLeft, args4: _traceTag);
            }
        }
#endif
        private Packet() { }
...
#if DEBUG
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} allocated with _headerLength {2}, _dataCapacity {3}", args0: _messenger?.ConnectionId, args1: _id, args2: _headerLength, args3: _dataCapacity);
#endif
        }
        ...
        public int TakeData(Packet packet, int size)
        {
            int dataSize = TakeData(packet._data, packet._headerLength + packet._dataLength, size);
            packet._dataLength += dataSize;
#if DEBUG
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} took data from Packet Id {2} dataSize {3}, _dataLength {4}", args0: _messenger?.ConnectionId, args1: _id, args2: packet?._id, args3: dataSize, args4: packet._dataLength);
#endif
            return dataSize;
        }
        ...
        public void AppendData(byte[] data, int size)
        {
            ...
            _dataLength += size;
#if DEBUG
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} was appended with size {2}, _dataLength {3}", args0: _messenger?.ConnectionId, args1: _id, args2: size, args3: _dataLength);
#endif
        }
        ...
        public int TakeData(byte[] buffer, int dataOffset, int size)
        {
            ...
            _dataOffset += size;
#if DEBUG
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} took data size {2}, _dataLength {3}, _dataOffset {4}", args0: _messenger?.ConnectionId, args1: _id, args2: size, args3: _dataLength, args4: _dataOffset);
#endif
            return size;
        }
        ...
        public void SetHeaderActive()
        {
            ...
            _headerLength = 0;
#if DEBUG
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} _dataLength {2} header set to active.", args0: _messenger?.ConnectionId, args1: _id, args2: _dataLength);
#endif
        }
...
#if DEBUG
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} _headerLength {2} and _dataLength {3} released.", args0: _messenger?.ConnectionId, args1: _id, args2: _headerLength, args3: _dataLength);
#endif
            _dataLength = 0;
            _dataOffset = 0;
            _headerLength = 0;
...
#if DEBUG
            SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} _dataLength {2} read from stream.", args0: _messenger?.ConnectionId, args1: _id, args2: _dataLength);
#endif
        }
        ...
        {
            _dataLength = await stream.ReadAsync(_data.AsMemory(0, _dataCapacity), cancellationToken);
            ...
            if (_dataLength == 0)
            {
                GlobalErrorHandler.Instance.LastError = new SqlNetworkError(Providers.TCP_PROV, 0, Constants.ConnTerminatedError, Strings.SNI_ERROR_2);
#if DEBUG
                SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.ERR, "Connection Id {0}, No data read from stream, connection was terminated.", args0: _messenger?.ConnectionId);
#endif
            }
        }

        /// <summary>
        /// Write data to a stream asynchronously or synchronously as per 'async' argument.
        /// </summary>
        /// <param name="stream">Stream to write to</param>
        /// <param name="callback">SNI Asynchronous Callback</param>
        /// <param name="provider">SNI provider identifier</param>
        /// <param name="async">Whether or not method was called from an async API.</param>
        /// <param name="cancellationToken">Cancellation Token</param>
        public async ValueTask WriteToStreamAsync(Stream stream, TdsAsyncCallback callback, Providers provider, bool async, CancellationToken cancellationToken)
        {
            ...
            try
            {
                if (async)
                {
                    await stream.WriteAsync(_data.AsMemory(0, _dataLength), cancellationToken).ConfigureAwait(false);
                }
                else
                {
...
#if DEBUG
                SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.INFO, "Connection Id {0}, Packet Id {1} _dataLength {2} written to stream.", args0: _messenger?.ConnectionId, args1: _id, args2: _dataLength);
#endif
            }
            catch (Exception e)
            {
                GlobalErrorHandler.Instance.LastError = new SqlNetworkError(provider, Constants.InternalExceptionError, e);
#if DEBUG
                SqlClientEventSource.Log.TrySNITraceEvent(nameof(Packet), EventType.ERR, "Connection Id {0}, Internal Exception occurred while writing data: {1}", args0: _messenger?.ConnectionId, args1: e?.Message);
#endif
